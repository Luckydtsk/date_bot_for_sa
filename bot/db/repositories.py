...
class ProfileRepo:
    ...
    async def upsert_complete(self, telegram_id: int, **fields: Any) -> Profile:
        ...
        if existing:
            ...
            existing.is_banned = existing.is_banned  # keep ban
            await self.session.commit()
    ...
    async def next_feed_candidate(
        ...
        *,
        ...
        # Кандидат тоже должен искать наш пол (или any)
        looking_back = or_(
    ...
    async def incoming_likes(self, viewer: Profile) -> list[tuple[Profile, Like]]:
        """Кто лайкнул нас, а мы ещё не ответили."""
...
class MatchRepo:
    ...
    async def create_if_needed(self, a: int, b: int) -> tuple[Match, bool]:
